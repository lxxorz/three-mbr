...
export class MinimumBoundingRectangle {
  ...
  private enhancedRotatingCalipers(hull: Vector2[]): {
    ...
    for (const direction of edges) {
      ...
      if (area < minArea) {
        ...
        // 计算中心点
        const center = new Vector2(((left + right) * direction.x) / 2 + ((bottom + top) * perpendicular.x) / 2, ((left + right) * direction.y) / 2 + ((bottom + top) * perpendicular.y) / 2)
  ...
  public grahamScan(points: Vector2[]): Vector2[] {
    ...
    for (let i = 2; i < sortedPoints.length; i++) {
      while (stack.length >= 2 && this.crossProduct(stack[stack.length - 2], stack[stack.length - 1], sortedPoints[i]) <= 0) {
        stack.pop()
      }
  ...
  private crossProduct(p1: Vector2, p2: Vector2, p3: Vector2): number {
    ...
    const v2 = new Vector2(p3.x - p1.x, p3.y - p1.y)
    return v1.cross(v2)
  }
